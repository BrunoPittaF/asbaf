...
const ManagePartner: React.FC = () => {
  ...
  const { partnerGlobal } = useGlobalContext();
  const { id } = useParams();
  const {
    register,
    handleSubmit,
    ...
    defaultValues: {
      cnpj: partnerGlobal.cnpj,
      name: partnerGlobal.name,
      cellphone: partnerGlobal.cellphone,
      email: partnerGlobal.email,
      instagram: partnerGlobal.instagram,
      website: partnerGlobal.website,
      note: partnerGlobal.note,
      isPartner: partnerGlobal.isPartner,
    },
  });
  ...
  const onSubmit = async (data: IPartner) => {
    ...
    formData.append('isPartner', data.isPartner ? 'true' : 'false');

    const response = await axios.put(`${baseURL}/partner/register/${id}`, formData);
          ...
          {partnerGlobal!.image && (
            <>
              ...
              <img
                ...
                src={baseURL + '/' + partnerGlobal!.image}
                alt={partnerGlobal!.name ?? ''}
              />
            </>
          )}
        </div>
